...
def h(x):
    ...
    for i in range(9):
        if x[i]!=8:
            hx+=abs(x[i]//3-i//3)+abs(x[i]%3-i%3)
    if x[0]==0 and x[1]!=8:
        hx+=6
    if x[1]==1 and x[2]!=8:
        hx+=6
    ...
    if len(OPEN)==0:
        ...
        if N==Sg:
            ...
            if ij%3<2:
                ...
                s4.append(n)
                s4.append(h(s4))
            if len(s1)!=0 and s1[:9] not in [i[0:9] for i in CLOSED] and s1[:9] not in [i[0:9] for i in OPEN]:
                s=copy.deepcopy(s1)
                OPEN.append(s)
            if len(s2)!=0 and s2[:9] not in [i[0:9] for i in CLOSED] and s2[:9] not in [i[0:9] for i in OPEN]:
                s=copy.deepcopy(s2)
                OPEN.append(s)
            if len(s3)!=0 and s3[:9] not in [i[0:9] for i in CLOSED] and s3[:9] not in [i[0:9] for i in OPEN]:
                s=copy.deepcopy(s3)
                OPEN.append(s)
            if len(s4)!=0 and s4[:9] not in [i[0:9] for i in CLOSED] and s4[:9] not in [i[0:9] for i in OPEN]:
                s=copy.deepcopy(s4)
                OPEN.append(s)      
